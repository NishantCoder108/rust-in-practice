...
fn main() {
    let name: &'static str = "nishant";
    let age = 25;
    let message = "Hello from main thread".to_string();


    println!("Name: {name}, Age: {age}");
    thread::spawn(move || { //when we need to move the variable to the thread, we need to use `move` keyword
        println!("Hello from another thread: {message}");
    }).join().unwrap();
