...
pub struct Borrow<'info> {
    ...
    pub supply_vault: Box<InterfaceAccount<'info, TokenAccount>>,
    #[account(
        mut,
        seeds = [b"collateral_vault", market_id.to_le_bytes().as_ref(), collateral_mint.key().as_ref()],
        bump
    ...
    pub collateral_mint: InterfaceAccount<'info, Mint>,
    #[account(mut)]
    pub user_supply_account: Box<InterfaceAccount<'info, TokenAccount>>, // User's account to receive borrowed supply_mint
    #[account(mut)]
    pub user_collateral_account: Box<InterfaceAccount<'info, TokenAccount>>, // User's account to provide collateral_mint
    #[account(mut)]
    pub user: Signer<'info>,
    /// CHECK: Oracle account for collateral pricing
    pub collateral_oracle: AccountInfo<'info>,
...
pub struct Liquidate<'info> {
    ...
    pub collateral_mint: InterfaceAccount<'info, Mint>,
    #[account(mut)]
    pub borrower_deposit: Box<Account<'info, UserDeposit>>,
    #[account(mut)]
    pub liquidator_supply_account: Box<InterfaceAccount<'info, TokenAccount>>,
    #[account(mut)]
