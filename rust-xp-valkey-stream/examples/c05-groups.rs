...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let consumer_g01_a_handle =
        run_consumer(&client, stream_name, group_01, "consumer_g01_a").await?;
    let consumer_g01_b_handle =
        run_consumer(&client, stream_name, group_01, "consumer_g01_b").await?;

    let consumer_g02_a_handle =
        run_consumer(&client, stream_name, group_02, "consumer_g02_a").await?;
            ...
            if let Some(reply) = res {
                for stream_key in reply.keys {
                    for stream_id in stream_key.ids {
                        ...
                        let res: Result<(), _> = con_reader
                            .xack(stream_name, group_name, &[stream_id.id])
                            .await;
                        if let Err(res) = res {
...
async fn create_group(
    ...
    let mut con = client.get_multiplexed_async_connection().await?;
    let group_create_res: Result<(), _> = con
